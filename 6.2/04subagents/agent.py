...
SUBTOOLS = [
    ...
    {"type": "function", "function": {"name": "todo", "description": "更新任务列表，跟踪多步骤任务进度。",
...
MATERTOOLS = [
    {"type": "function", "function": {"name": "agent", "description": "启动一个拥有全新上下文的子代理。它共享文件系统，但不共享对话历史。",
     "parameters": {"type": "object", "properties": {"prompt": {"type": "string"}, "description": {"type": "string", "description": "任务的简短描述"}}, "required": ["prompt"]}}},
]
...
def run_subagent(prompt: str) -> str:
    sub_messages = [
        {"role": "system", "content": "你是一个在{WORKDIR}下的code subagent. 完成给定的任务，之后总结你的发现"},
    ...
    while True:
        current_round += 1
        lined_print(f"SubAgent Calling LLM (round {current_round})")
        ...
        if response.choices[0].message.reasoning_details[0]['text'] != "":
            framed_print(f"SubAgent Thinking", response.choices[0].message.reasoning_details[0]['text'], "info")

        if response.choices[0].message.content != "":
            framed_print(f"SubAgent Answer", response.choices[0].message.content, "info")
        ...
        if response.choices[0].message.tool_calls != None:
            ...
            for tool_call in response.choices[0].message.tool_calls:
                if tool_call.function.name == "run_bash":
                    ...
                    arguments_dict = json.loads(tool_call.function.arguments)
                    result = TODO.update(arguments_dict['items'])
                else:
...
def agent_loop(messages):
    ...
    while True:
        ...
        if response.choices[0].message.tool_calls != None:
            ...
            for tool_call in response.choices[0].message.tool_calls:
                if tool_call.function.name == "agent":
                    arguments_dict = json.loads(tool_call.function.arguments)
                    result = run_subagent(arguments_dict['prompt'])
                else:
                    result = "Error: Unknown tool"
...
if __name__ == "__main__":
    ...
    history = [{"role": "system", "content": "你是一个在{WORKDIR}下的code agent. 使用agent工具去委派子任务"}]
    while True:
        try:

...
def generate_command(state: AgentState):
    ...
    if iter == 0:  # 第一次生成
        prompt = COMMAND_PROMPT.format(
            user_query=state["user_query"], best_command="无", reflection="无"
        )
    else:
        prompt = COMMAND_PROMPT.format(
...
def reflect_and_optimize(state: AgentState):
    ...
    prompt = REFLECTION_PROMPT.format(
        command=state["best_command"], user_query=state["user_query"]
    )
    response = llm_Tongyi.invoke(prompt)
    content = response.content
    if "无建议" in content or "无需优化" in content:
        return {
            "reflection": "已经最优,无需优化",
        }
...
ret = graph.invoke(
    {
        "user_query": "使用docker创建nginx容器,端口映射8080:80",
        "best_command": "",
        "reflection": "",
        "iterations": 0,
    }

...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "background_run", "description": "在后台线程运行命令，然后立刻返回task_id",
        ...
        if response.choices[0].message.tool_calls != None:
            ...
            for tool_call in response.choices[0].message.tool_calls:
                if tool_call.function.name == "run_bash":
                    ...
                    arguments_dict = json.loads(tool_call.function.arguments)
                    result = BG.run(arguments_dict['command'])
                elif tool_call.function.name == "check_background":
                    arguments_dict = json.loads(tool_call.function.arguments)
                    result = BG.check(arguments_dict.get('path'))
                else:
...
if __name__ == "__main__":
    ...
    history = [{"role": "system", "content": "你是在 {WORKDIR} 目录下的Code Agent。对于需要长时间运行的命令，必须使用background_run工具，使其在后台运行"}]
    while True:
        try:

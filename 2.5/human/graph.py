...
async def run_graph():
    async def llm_node(state: HumanState):
        messages = [
            SystemMessage(
                content="你是一个仓库管理员,根据用户的要求回答相关的价格和库存信息"
            ),
    ...
    async def tool_node(state: HumanState):
        ...
        for tool_call in state["messages"][-1].tool_calls:
            ...
            state["messages"].append(
                ToolMessage(content=call_tool_ret, tool_call_id=tool_call["id"])
            )
    ...
    ret = await graph.ainvoke(
        {"query": "我想买一些苹果，总共需要多少钱"}, config=thread_config
    )

    if (
        ret["messages"][-1].tool_calls
        and ret["messages"][-1].tool_calls[0]["name"] == "ask_user"
    ):

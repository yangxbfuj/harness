...
async def run_graph():
    async def plan_node(state: PlanState):
        # 调用 LLM
        response = await llm_Tongyi.ainvoke([SystemMessage(content=PLAN_PROMPT),state["messages"][0]])
    ...
    ret = await agent.ainvoke({"plan":"", "messages": [HumanMessage(content="帮我写一份潮汕旅游攻略")]},config=thread_config)
